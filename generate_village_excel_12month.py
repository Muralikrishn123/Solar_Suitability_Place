...
def fetch_monthly_gee_data(coords_df):
    """
    Optimized to fetch static features (Elevation, Slope, Aspect) 
    AND all 12 months of Solar/Temp in one pass per batch.
    """
    ...
    terrain = ee.Algorithms.Terrain(elev) # bands: elevation, slope, aspect
    
    # Combined image starts with terrain
    combined_img = terrain.select(['elevation', 'slope', 'aspect'])
    
    # Add 12 months of Solar and Temp
    print("Preparing 12-month image layers...")
    for m in range(1, 13):
        # Solar
        ...
        combined_img = combined_img.addBands(srad).addBands(temp)
    
    print(f"Running multi-month reduceRegions for {len(coords_df)} villages...")
    ...
    for f in results['features']:
        ...
        # Expand into 12 rows per village
        for m in range(1, 13):
            srad_raw = props.get(f'srad_{m}') or 0
            srad_w_m2 = srad_raw * 0.1
            solar_val = (srad_w_m2 * 24) / 1000 if srad_w_m2 else 5.5
            
            temp_raw = props.get(f'temp_{m}')
...
def main():
    ...
    # We'll stick to a chunk size that handles 24 bands * 300 villages effectively
    # 24 bands * 300 = 7,200 data points per call. 
    chunk_size = 200 
    limit = len(coords)
    coords_subset = coords.head(limit)
    
    print(f"Starting 12-Month Analysis for {len(coords_subset)} villages...")
    
    csv_temp = "village_suitability_full_12month_temp.csv"
    
    for start in range(0, len(coords_subset), chunk_size):
        end = min(start + chunk_size, len(coords_subset))
        print(f"\n[Month Analysis] Progress: {start}/{len(coords_subset)} villages...")
        chunk = coords_subset.iloc[start:end]
        
        try:
            batch_df = fetch_monthly_gee_data(chunk)
            ...
            batch_df['verdict'] = batch_df['cuf_prediction'].apply(get_verdict)
    ...
    print("\n--- Processing Complete. Converting to Excel ---")
    final_df = pd.read_csv(csv_temp)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    report_name = f"village_suitability_12month_report_{timestamp}.xlsx"
    final_df.to_excel(report_name, index=False)
    print(f"Full 12-month report generated: {report_name}")
